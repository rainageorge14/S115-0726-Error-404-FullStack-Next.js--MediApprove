      ...
      <div className={`flex flex-col w-full gap-1.5 ${containerClassName}`}>
        <div className="flex items-center justify-between">
          {label && (
            <label
              htmlFor={id}
              className="text-xs font-semibold tracking-wide text-dark-navy/80 dark:text-dark-navy/90"
            >
              {label}
            </label>
          )}
        ...
        <div className="relative flex items-center">
          <input
            ...
            id={id}
            type={inputType}
            className={`w-full px-4 py-3 text-sm text-dark-navy bg-white border border-border-color rounded-xl outline-hidden transition-all duration-200 placeholder:text-gray-400 focus:border-primary focus:ring-3 focus:ring-primary/10 ${
